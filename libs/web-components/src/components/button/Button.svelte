...
<script lang="ts">
  ...
  $: isIconDark = type === "primary" || type === "start"
...
>
  ...
  {:else}
    {#if leadingicon}
      <goa-icon id="leading-icon" type={leadingicon} inverted={isIconDark} />
    {/if}
    <div class="text">
      <slot />
    </div>
    {#if trailingicon}
      <goa-icon id="trailing-icon" type={trailingicon} inverted={isIconDark} />
    {/if}
  {/if}
</button>
...
<style>
  ...
  .text {
    padding-bottom: 0.1rem;  /* acumin font requires this to allow for vertical alignment  */
  }
